...
/*		} else {
	...
	*/
	function _destruct() {
		global $logging, $mid, $motions, $recesses, $roles, $speakers, $users;
		...
		if ($this->status == 'scheduled') {
			if ($this->start < strtotime('tomorrow'))
				$day = "";
			elseif ($this->start < strtotime('midnight +2 days'))
				$day = " tomorrow";
			elseif ($this->start < strtotime('next year january 1'))
				$day = date(" M dS", $this->start);
			else
				$day = date(" M dS, Y", $this->start);
			
			say("The meeting scheduled to begin$day at " . date('h:i a T', $this->start) . " has been cancelled.");
			return;
		}
